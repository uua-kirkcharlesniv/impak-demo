            ...
            <div class="bg-white rounded shadow-lg overflow-auto max-w-lg w-full max-h-full"
                ...
                <div class="px-5 py-4">
                    <nav class="flex border-b border-gray-100 text-sm font-medium mb-5">
                        <button wire:click="$set('tab', 'create')" class="-mb-px border-b {{ $tab == 'create' ? 'border-current text-cyan-500' : 'border-transparent hover:text-cyan-500' }} p-4">
                            Create
                        </button>

                        <button wire:click="$set('tab', 'import')" class="-mb-px border-b {{ $tab == 'import' ? 'border-current text-cyan-500' : 'border-transparent hover:text-cyan-500' }} p-4">
                            Import
                        </button>
                    ...
                    @if ($tab == 'create')
                        ...
                        <div class="space-y-3">
                            <div class="grid gap-6 md:grid-cols-2">
                                <div>
                                    <label class="block text-sm font-medium mb-1" for="name">First Name<span
                                            class="text-rose-500">*</span></label>
                                    <input id="first_name" class="form-input w-full px-2 py-1" type="text"
                                        required />
                                </div>
                                <div>
                                    <label class="block text-sm font-medium mb-1" for="name">Last Name<span
                                            class="text-rose-500">*</span></label>
                                    <input id="last_name" class="form-input w-full px-2 py-1" type="text" required />
                                </div>
                            </div>
                            <div>
                                <label class="block text-sm font-medium mb-1" for="email">Date of Birth<span
                                        class="text-rose-500">*</span></label>
                                <input id="dob" class="form-input w-full px-2 py-1" type="date" required />
                            </div>
                            <div>
