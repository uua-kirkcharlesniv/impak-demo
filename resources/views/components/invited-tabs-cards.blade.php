    ...
    <div class="flex flex-col h-full">
        ...
        <div class="grow p-5">
            ...
            <div class="relative">
                <div class="absolute top-0 right-0 inline-flex" x-data="{ open: false }">
                    ...
                    <div class="origin-top-right z-10 absolute top-full right-0 min-w-36 bg-white border border-slate-200 py-1.5 rounded shadow-lg overflow-hidden mt-1" @click.outside="open = false" @keydown.escape.window="open = false" x-show="open" x-transition:enter="transition ease-out duration-200 transform" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-out duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" x-cloak>
                        <ul>
                            <li>
                                <a href="{{ route('delete-invite', $member->id) }}" class="font-medium text-sm text-rose-500 hover:text-rose-600 flex py-1 px-3" href="#0" @click="open = false" @focus="open = true" @focusout="open = false">Remove</a>
                            </li>
                        </ul>
                    </div>
                </div>
            ...
            <header>
                ...
                <img class="rounded-full" src="https://ui-avatars.com/api/?name={{ $member->first_name }} {{ $member->last_name }}" width="64" height="64" alt="{{ $member->first_name }} {{ $member->last_name }}" />
                {{-- </a> --}}
        </div>
        ...
        <div class="flex justify-center items-center"><span class="text-sm font-medium text-slate-400 -mt-0.5 mr-1">
                {{ $member->email }}
            </span></div>
        <div class="flex mt-4 justify-center items-center">
            @php
            /// If status is pending, use the amber color
            /// Else if status is pending, use the green color
            /// Else use the slate color
            /// Use switch case
            $color = 'slate';
            ...
            @endphp
            <span class="text-md font-medium text-{{ $color }}-600 -mt-0.5 mr-1">
